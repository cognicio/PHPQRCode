...
use Code\Library\Bitstream;
use Exception;
use Code\Libary\Spec;
use Code\Libary\InputItem;
...
class Input
{
    ...
    {
        if ($version < 0 || $version > Spec_VERSION_MAX || $level > QR_ECLEVEL_H) {
    ...
    {
        if ($version < 0 || $version > Spec_VERSION_MAX) {
            throw new Exception('Invalid version no');
        }
    ...
    public function estimateVersion()
    {
        ...
        do {
            ...
            $version = \Code\Libary\Spec::getMinimumVersion((int) (($bits + 7) / 8), $this->level);
            if ($version < 0) {
                return -1;
            }
        } while ($version > $prev);
    ...
    {
        $payload = $bits - 4 - \Code\Libary\Spec::lengthIndicator($mode, $version);
        switch ($mode) {
            case QR_MODE_NUM:
